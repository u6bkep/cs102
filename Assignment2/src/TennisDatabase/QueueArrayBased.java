...
public class QueueArrayBased<T> {
   ...
   private int front; // Index of the player at the front of the queue.
   private int back; // Index of the player at the back of the queue.
   ...
   // Method to insert an player at the back of this queue.
   public void enqueue( T item ) {
      ...
      // Insert input player in array at back.
      this.back = ( this.back + 1 ) % this.sizePhysical; // Update index of player at back.
      this.array[ this.back ] = item;
      this.sizeLogical++;
   }
   
   // Method to extract an player from the front of this queue.
   public T dequeue() {
      if( isEmpty() ) { return null; } // Return null if queue is empty.
      else {
         T itemAtFront = this.array[ this.front ]; // Store apart player at front of queue.
         this.array[ this.front ] = null; // Totally unnecessary.
         this.front = ( this.front + 1 ) % this.sizePhysical; // Update index of player at front.
         this.sizeLogical--;
         return itemAtFront; // Return player stored apart.
      }
   }
